...
int main(int argc, char *argv[]) {
	...
	for (int row = 0; row < HEIGHT; row++) {
		int bar_width = rand() % WIDTH;//each row
		for (int col = 0; col < WIDTH; col++) {
			if (WIDTH - col >= bar_width) {
				...
				row_pointers[row][col].g = 100; // green
				row_pointers[row][col].b = 100; // blue
				row_pointers[row][col].a = 10; // alpha (opacity)
			}
		}
	}
